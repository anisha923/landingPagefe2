...
const SignUp = ( {}) => {
    ...
    const [showIncorrectOtpPopup, setShowIncorrectOtpPopup] = useState(false);
    ...
    const str = "Buy Medical Insurance Plans"
    ...
    setShowOtpPopup(true);
          ...
          axios.post('http://13.200.66.57:7000/user/register', data)
            .then(response => {
              // Handle the response from the server
              console.log(response.data.responseResult.mobileNumber);
              Cookies.set('mobileNumber', response.data.responseResult.mobileNumber)
            })
            .catch(error => {
      ...
      const handleResendOtp = (e) => {
        e.preventDefault();
        const otpResendUrl = 'http://13.200.66.57:7000/user/resend';
        const data = {mobileNumber:Cookies.get('mobileNumber')}


        axios.post(otpResendUrl,data)
          .then((response) => {
            // Handle the response from the server after resending OTP
      ...
      const handleOtpSubmit = (e) => {
        ...
        const otpVerifyUrl = 'http://13.200.66.57:7000/user/otpVerify';
        const data = { otp: otpValue, mobileNumber: Cookies.get('mobileNumber') };
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.data.responseCode === 200) {
              ...
              setShowThankYouPopup(true); // Display the "Thank You" popup
              Cookies.remove('mobileNumber');
              // history.push('/home');
              // setShowIncorrectOtpPopup(true); // Display the "Incorrect OTP" popup
...
<div style= {{marginTop:'30px'}} className="header">
      ...
      <div  style = {{display:'flex' , flexWrap:'wrap',justifyContent:'space-between',rowGap:'200px'}} className="middle">
      <div  style = {{flexBasis:'50%'}} className="left-portion">
       <h1 style={{color:"blue"}}>Buy Medical Insurance Plans</h1>
       <h4 style={{color:"blue"}}> &#x20b9; 5 Lacs Health Cover Starts @ &#x20b9;15/day* </h4>
        <div style={{marginLeft:'0px'}} className="img">
        <img style ={{width:"500px" ,height:"500px"}} src="./secondpage.png" />
       </div>
      </div>

        <div style = {{marginRight:'50px'}} className="login_page" >
            <div className="form_width">
                ...
                >
                    ...
                    <Form className="form_box_signup" onSubmit={handleSubmit}>
          ...
          <div className="d-flex">
                                          <input
                                            ...
                                            style={{ marginRight: '10px',width:'120px',height:'40px',borderRadius:'10px' }}
                                          />
                                          <select
                        ...
                        <button  style={{ width: '150px', padding: '10px', borderRadius: '60px', backgroundColor: 'blue', color: 'white', fontWeight: 'bold',margin:'5px' }} type="submit" disabled={isSubmitting}>
        Get Quotes
      </button>
                           
      <br></br>
                     <label style={{fontFamily:'-moz-initial',fontSize:'13px',marginBottom:'100px'}}>
            ...
            {showOtpPopup && (
        <div className="otp-popup" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 9999, background: 'white', padding: '20px', borderRadius: '10px', maxWidth: '400px', width: '90%', textAlign: 'center' }}>
          <h2 style={{ fontSize: '24px', marginBottom: '20px' }}>OTP Verification</h2>
          <button style={{ borderRadius:'10px',  position: 'absolute', top: '10px', right: '10px', cursor: 'pointer', fontWeight: 'bold', background: 'blue', border: '1px solid black' }} onClick={handleCloseOtpPopup}>Close</button>
          <form onSubmit={handleOtpSubmit}>
            ...
            />
            <button type="submit" style={{ backgroundColor: 'blue', color: 'white', padding: '10px 20px', borderRadius: '5px', border: 'none', fontWeight: 'bold' }}>Verify</button>
            <button
              type="button"
      ...
      {showThankYouPopup && (
        <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 9999, background: 'white', padding: '20px', borderRadius: '10px', maxWidth: '300px', width: '90%', textAlign: 'center' }} className="thankyou-popup">
          <h2>Thank You for Connecting with Us!</h2>
          <button onClick={() => setShowThankYouPopup(false)}>Close</button>
        </div>
      )}

      {showIncorrectOtpPopup && (
         <div className="incorrect-otp-popup" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 9999, background: 'white', padding: '20px', borderRadius: '10px', maxWidth: '300px', width: '90%', textAlign: 'center' }}>
          <h2>Incorrect OTP</h2>
          <p>Please enter the correct OTP.</p>
          <button onClick={() => setShowIncorrectOtpPopup(false)}>Close</button>
        </div>
      )}
         </div>
        ...
        <div style={{marginTop:'150px'}}> <h1  style={{alignContent:'center' , marginLeft:'100px' , fontWeight:'bold' , color:'darkblue'}}>Why Policy Mall?</h1></div>
        <div className="pagea">
           <div style={{backgroundColor:'lightblue' , boxShadow:'0 15px 0 lavender , inset 0 -15px 0 rgb(0,0,139),0 20px'}} className="cards" ><h4 style={{ fontWeight:'20px',textAlign:'center',marginTop:'80px'}}>Coverage Against Covid'19</h4></div>
           <div style={{backgroundColor:'lightblue' , boxShadow:'0 15px 0 lavender  , inset 0 -15px 0 rgb(0,0,139),0 20px'}} className="cards"><h4 style={{ fontWeight:'20px',textAlign:'center',marginTop:'80px'}}>Choose Customize Riders</h4></div>
           <div style={{backgroundColor:'lightblue' , boxShadow:'0 15px 0 lavender  , inset 0 -15px 0 rgb(0,0,139),0 20px'}}  className="cards"><h4 style={{ fontWeight:'20px',textAlign:'center',marginTop:'80px'}}>Cashless Treatments*</h4></div>
           <div style={{backgroundColor:'lightblue' , boxShadow:'0 15px 0 lavender  , inset 0 -15px 0 rgb(0,0,139),0 20px'}} className="cards"><h4 style={{ fontWeight:'20px',textAlign:'center',marginTop:'80px'}}>Tax Benefits u/s 80D</h4></div>
           <div style={{backgroundColor:'lightblue' , boxShadow:'0 15px 0 lavender  , inset 0 -15px 0 rgb(0,0,139),0 20px'}} className="cards"><h4 style={{ fontWeight:'20px',textAlign:'center',marginTop:'80px'}} >IRDA Approved Plans</h4></div>
           <div style={{backgroundColor:'lightblue' , boxShadow:'0 15px 0 lavender  , inset 0 -15px 0 rgb(0,0,139),0 20px'}} className="cards"><h4 style={{ fontWeight:'20px',textAlign:'center',marginTop:'80px'}}>Best Claim Settlement Ratio*</h4></div>
        </div>
           ...
           <div  className="logoi">
            ...
            <div className="sp">
            <img   style={{width:'160px', height:'110px'}} src="./icici.png" />
            </div>
            <div className="sp">
            <img src="./sbi.png" />
            </div>
...
advantages of comparing health plans to maximize your benefits:</h5>
            ...
            <p style={{margin:'30px' ,fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Coverage Amount/Sum Insured (SI)</h6>Choose a lower premium with higher Coverage/Sum Insured to get suitable medical care in times of
emergencies. Don’t under insure to save on premiums.</p>
                
            </div>
            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
                
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Co Pay</h6>With Co Pay, the insured agrees to pay a fraction of the total medical cost from his or her own pocket, and the balance will be paid out
of the insurance policy. Choose the benefits of Co Pay and get the best deal that fits your budget and coverage needs. </p>
                
            </div>
            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
               
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Network Hospitals</h6>
            Choose the plan with a nearby hospital that has well-equipped facilities for cashless treatment.</p>
                
            </div>
            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
               
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Pre-existing Cover</h6>In every policy, there is a waiting period for Pre-existing disease, Go for lowest waiting period or no waiting period at all</p>
                
            </div>
            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
                
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>No-Capping</h6>Even though a lot of plans fix an upper limit of expenses like in case of room rent or doctor visit, certain plans come with no
capping!</p>
               
            </div>
            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
              
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>No claim bonus</h6>NCB one of the most important health insurance benefit. You can get a 10 to 100 per cent additional sum insured for each policy year without a health insurance claim as a 'No-claim Bonus'.</p>
                
            </div>

            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
               
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Critical Illnesses Rider</h6>Health insurance plans offer coverage for critical illnesses such as cancer, paralysis, kidney disorders, diabetes, and heart related problems, which can require costly treatment and management.</p>
            ...
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Restoration of Benefits</h6>If you exhausted the full sum insured due to hospitalization, some plans will restore the entire amount within the same policy year at no extra cost.</p>
               
            </div>

            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
               
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Additional Riders</h6>Buy the best health insurance plan and add additional riders to cover specific risks as per your requirements.</p>
            
            </div>

            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
                
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Day Care Procedures</h6>Every Insurance company provides some specific plans for day care treatment which did not require 24 hours or more of hospitalization.</p>
                
            </div>

            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
                
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>OPD Benefits</h6>Most of the insurers provide you comprehensive health cover by reimbursing your claims for Out Patient Department (OPD)</p>
                
            </div>

            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
                
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Annual Health Check-ups</h6>Several insurers provides complimentary annual health check-ups. These check-ups can help detect potential health issues early on and reducing future medical expenses.</p>
                
            </div>

            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
            
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Pre And Post Hospitalization</h6>Health insurance not only covers post-hospitalization costs, but it also covers pre-hospitalization costs.</p>
                
            </div>


            <div  style = {{backgroundColor:'lavender' , margintop:'200px', borderRadius:'50px'}} className="line">
            <p style={{margin:'30px',fontFamily:'-moz-initial'}}><h6 style={{color:'blue', fontWeight:'bold'}}>Customer Service</h6>Lastly, consider the quality of customer service offered by the insurer, as this can impact your overall experience with the
policy.</p>
            </div>
         ...
         <div  style={{display:'flex ',justifyContent:'center'}}   className="imp">
         <p style={{flexDirection:'row' ,fontFamily:'-moz-initial',fontWeight:'bold',color:'black',fontWeight:'20px',textAlign:'center',maxWidth:'600px',margin:'0 auto'}}>Trinity Reinsurance Brokers Ltd.<br></br> B2,Sector 7,Noida,Uttar Pradesh 201301<br></br> Licensed By-IRDAI/353 Valid till 04 Apr 2025<br></br>CIN: U01111DL1986PLC025441<br></br>Grievance Cell : Grievance Officer (+91-7428266733)<br></br></p>
          </div>
        <p style={{ display:'flex',flexWrap:'wrap',flexDirection:'row',flexBasis:'50%' ,fontFamily:'-moz-initial',fontWeight:'bold',color:'black',fontWeight:'20px',width:'100%'}}>DISCLAIMER: *Subject to underwriting guidelines of Insurance companies
Insurance is the subject matter of solicitation. Visitors are hereby informed that their information submitted on the website
may be shared with insurers. For more details on risk factors, terms and conditions please read policy wording carefully before
concluding a sale. Tax benefits are subject to changes in taxation laws. Product information is authentic and solely based on the
information received from the Insurer. Terms and Conditions shall apply for standard insurance plans.</p> 
        </div>
